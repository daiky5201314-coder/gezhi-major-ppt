...
本文件只管**模式 B 的专业类课程**（主技能称“专业大类”，如“电子信息类”）生成后的保存位置。研究、页面结构、写稿和事实核验继续按主技能原规则执行。普通咨询不生成文件；单个本科专业课程暂时只保存本地，不上传。**三份 Markdown 仅为上传而生成：默认不保留本地文件，上传核验成功后即删除，交付只提供飞书链接。**
...
4. 上传时向飞书接口或连接器**传入精确的 `content.token` 作为父文件夹**，用带版本号的显式文件名直传（内容读自原稿），不在本地创建版本副本。上传后重新列出该目标文件夹或读取每份文件的元数据，逐一核对文件名、类型、token 和 `parent_token == content.token`。只拿到“上传成功”消息或文件链接不足以确认归档正确。
5. 返回三份文件各自的飞书链接及目标`内容稿`文件夹链接，不提供本地路径（本地临时稿上传核验成功后即删除；失败时报告保留的临时稿路径）。若无法访问映射、授权身份不符、上传接口无法指定父文件夹、核验失败或只有部分成功，说明实际结果，不得声称飞书交付完成；保留已有文件，不覆盖或盲目重传。

这套流程适用于所有 agent 和运行环境。任何 agent 都应使用当前环境已授权的飞书工具完成身份核对、列目录、上传和核验；不要求特定厂商的连接器。工具缺少必需的账号、目标文件夹或文件元数据时，停止保存并说明缺口。任何历史目录或默认目录都不能替代映射中的 `content.token`。生成的三份 Markdown 仅用于上传，核验成功后即删除；交付只提供飞书链接。
...
3. `python scripts/feishu_route_guard.py plan --class-name <专业类名称或四位代码> --proof route-proof.json --files <原稿1.md> <原稿2.md> <原稿3.md> --out delivery-plan.json`：核对账号、实时目录层级、版本和原稿文件名，生成同版本的本地副本及交付计划。失败则停止，不能上传。**默认直传方式可跳过本地版本副本**：`local_versioned` 仅用于需要副本的脚本/其他环境；直传时以原稿内容配合带版本号的 `--name` 上传，内容与之等价。
4. 用飞书工具逐份上传（直传方式：`--name <带版本号文件名> --content @<原稿>`），明确指定计划中的 `folder_token` 作为父文件夹。每上传一份，将该次工具返回的文件名和 token 保存为 `upload-result.json`，再次读取目标 `内容稿` 的完整清单并保存为 `after-list.json`。运行 `python scripts/feishu_route_guard.py verify --plan delivery-plan.json --results upload-result.json --after after-list.json --name <本次文件名>`。只有返回 `ok: true` 才上传下一份；失败立即停止，逐份报告状态。
...
正式执行时，脚本从目标“内容稿”和本地输出目录已有的 `Vn.0` 文件中取最高主版本号加 1，默认三份共用同一版本。未标版本的历史稿不占用 `V1.0`；例如旧文件 `电子信息类-PPT内容稿.md` 保留，新稿从 `电子信息类-PPT内容稿-V1.0.md` 开始。脚本在本地创建带版本号的副本，用 `lark-cli markdown +create` 上传原生 Markdown，不覆盖或删除旧稿。**此副本机制供需要本地副本的环境复用；本机默认采用直传方式（`--name <带版本号文件名> --content @<原稿>`），不创建本地版本副本。**

每份文件创建后，脚本核对 CLI 返回的文件名、字节数、token，并重新读取目标“内容稿”目录验证文件的名称、类型、token 和父目录。若部分上传失败，保留临时稿和已经成功的云端文件，报告实际结果；不要盲目重复上传。结束时只向用户提供版本号、飞书文件链接和“内容稿”文件夹链接；上传核验成功的本地临时稿即删除，不提供本地路径（失败时报告保留的临时稿路径）。没有实际上传成功时，不能声称云端交付完成。
